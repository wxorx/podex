	...
	;;
	;; ATTiny2313 variant -- targets ATTiny2313-20PU with 18.432MHz crystal.
	;; The ATtiny2313 is register-compatible with the AT90S2313 for every
	;; register used here: UART UBRRL/UCSRB/UCSRA/UDR sit at the same I/O
	;; addresses (0x09-0x0c) with identical bit definitions, PORTD/PIND and
	;; PORTB/PINB are unchanged, SPL is at 0x3d and RAMEND is 0xdf. Only the
	;; crystal-dependent constants below differ from podex.s.
	;;
	...
	;; +------------------------+
	;;
.equ	POD_XTAL640_A,	18	; POD_XTAL640_A/POD_XTAL640_B = f_pod/(640MHz)
.equ	POD_XTAL640_B,	625	; (for precise computation of ExtendedSpeed spec. in v.4.7)
	;;
.equ	BAUD_RATE,	230400 ;115200
...
.equ	RST_DD,	DDRB
	;; RS232
	;; NOTE: this build is wired directly to an FT232R breakout with the
	;; handshake lines name-matched (PD2 -> FT232R pin 3 RTS#, PD3 -> FT232R
	;; pin 11 CTS#), so the pod's pin roles are swapped w.r.t. the MAX232
	;; schematic: the pod's output ("CTS of PC") is on PD3, its input
	;; ("RTS of PC") is on PD2.  Revert this swap for the MAX232 schematic.
.equ	RTS_P,	PORTD
.equ	CTS_I,	PIND
.equ	RTS_B,	PD3		; CTS of PC (drives FT232R CTS# input)
.equ	CTS_B,	PD2		; RTS of PC (reads FT232R RTS# output)
.equ	RTS_DD,	DDRD
.equ	CTS_DD,	DDRD
	;; etc.
.equ	IO_PORT_P,	PORTB
...
vector_table:
	...
	.string	"attiny_podex 68HC12 BDM <mp@duch.cz>"
	.align	2			; keep start word-aligned (string length is odd now)
...
pod_ext_03_lp:
	...
	;add	r21,1		; wAddress++
	;adc	r20,0
	; --- FIX: Increment address by 2 for word alignment ---
	subi	r21, 0xFE	; Add 2 to low byte
	sbci	r20, 0xFF	; Add 1 (plus carry) to high byte
	; -------------------------------------------------------
	subi	r19,1
	brsh	pod_ext_03_lp
	subi	r18,1
...
pod_ext_07:
    ...
    ; ------------------------------------------------------
    sts	XSPEED_HI,	r20	; Fixed: High byte to HI variable
    sts	XSPEED_LO,	r21	; Fixed: Low byte to LO variable
    ; ------------------------------------------------------
